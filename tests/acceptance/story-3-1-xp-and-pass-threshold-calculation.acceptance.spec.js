...
test.describe("Story 3.1 acceptance", () => {
    test("computes XP and pass summary from recorded outcomes", async ({ page }) => {
        ...
        await expect(page.getByTestId("challenge-summary-percentage")).toHaveText("98%");
        await expect(page.getByTestId("challenge-summary-xp-gate")).toHaveText("88/72");
        ...
        await expect(page.getByTestId("challenge-summary-first-try-accuracy")).toHaveText("89%");
        await expect(page.getByTestId("challenge-summary-corrected-mistakes")).toHaveText("1");
        await expect(page.getByTestId("challenge-summary-accuracy-card")).toHaveAttribute("data-performance-tone", "excellent");
        await expect(page.getByTestId("challenge-summary-first-try-accuracy-card")).toHaveAttribute("data-performance-tone", "good");
        await expect(page.getByTestId("challenge-summary-corrected-mistakes-card")).toHaveAttribute("data-performance-tone", "good");
