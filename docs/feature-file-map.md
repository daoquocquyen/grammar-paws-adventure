...
| F-03 | Challenge screen | Partial | `app/challenge/page.js`, `src/ui/stitch/screen4-game-challenge.html` | React route `/challenge` exists; legacy `/screen4-game-challenge` is redirected via `next.config.mjs`. Full challenge engine and scoring logic remain pending. |
