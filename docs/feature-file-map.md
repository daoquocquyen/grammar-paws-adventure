...
| F-02 | Grammar Topic Intro + voice controls | Partial-High | `app/topic-intro/page.js`, `src/lib/topicCatalog.js`, `src/components/HeaderBlock.js`, `src/ui/stitch/screen3-grammar-topic-intro.html` | Canonical React route is `/topic-intro`; legacy `/screen3-grammar-topic-intro` is redirected via `next.config.mjs`. Implements anchored intro composition, shared header reuse, dynamic aspect-card rendering from centralized topic metadata, loading/error/content states, and single speaker on/off voice control. |
| F-03 | Challenge screen | Partial | `app/challenge/page.js`, `src/lib/topicCatalog.js`, `src/lib/challengeQuestionSelection.js`, `src/ui/stitch/screen4-game-challenge.html` | React route `/challenge` exists; legacy `/screen4-game-challenge` is redirected via `next.config.mjs`. Question selection now derives aspect IDs from shared topic metadata to keep intro/challenge alignment; full challenge engine and scoring logic remain pending. |
